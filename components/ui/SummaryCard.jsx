...
export default function SummaryCard({ label, value, sublabel, icon: Icon, accent = "gold", compact = false }) {
  ...
  return (
    <div className={`rounded-xl border border-border-subtle bg-navy px-5 ${compact ? "py-2.5" : "py-4"} flex flex-col gap-1`}>
      ...
      <p className={`font-heading ${compact ? "text-xl sm:text-2xl" : "text-2xl sm:text-3xl"} font-bold ${accentClass}`}>{value}</p>
