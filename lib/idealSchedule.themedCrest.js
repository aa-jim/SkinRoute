...
function simulateThemedCrestFixedShape(event, packsData, resources = {}, extraDiaByDay = {}, rechargeDiaByDay = {}) {
  ...
  for (const phase of supply) {
    ...
    const data = { phase: phase.phase, startDay, endDay, rechargeTasks, spendTasks, loginTokens: loginTask?.tokens ?? 0, rechargeTotalTokens: rechargeTasks.reduce((s, t) => s + t.tokens, 0), highestRechargeThreshold: rechargeTasks.length > 0 ? rechargeTasks[rechargeTasks.length - 1].threshold : 0 };
    phaseByOpenDay.set(startDay, data);
  ...
  for (let day = 1; day <= duration; day++) {
    ...
    // Start-day notes (recharge + don't-claim instruction) — before 1x daily
    if (activePhaseData && phaseOpenDays.has(day)) {
    ...
    // Check spend thresholds after daily draw
    checkSpendThresholds(row);

    const closingPhase = phaseByCloseDay.get(day);
    if (closingPhase) {
      // Check spend thresholds before extra singles
      checkSpendThresholds(row);
      ...
      // Check spend thresholds after extra singles
      checkSpendThresholds(row);
...
function buildThemedCrestPlanWithPacks(event, packsData, targetDraws, resources = {}, extraDiaByDay = {}, rechargeDiaByDay = {}) {
  const sim = simulateThemedCrestFixedShape(event, packsData, resources, extraDiaByDay, rechargeDiaByDay);
  const cost1xFull = event.draw_cost_1x;
