...
function buildStandardPdf(doc, plan, event) {
  ...
  const targetLabel = hasTarget && !isBingo
    ? (hasOutfit1Header
  ...
  const cardList = [
    ...
    [
      "Diamonds Needed",
      ((isCollector || plan.eventType === "themed_crest") ? (plan.daySchedule?.totals?.dia ?? 0) : (plan.totalDiamondsForPlan ?? 0)).toLocaleString(),
      null,
    ],
  ];
  if (isCollector) {
  ...
  if (!isBingo) {
    cardList.push([
      "Target Skin",
      hasOutfit1 ? `${plan.target.skin.name} + ${plan.target.skin.outfit1_variant.name}` : plan.target?.skin?.name ?? "",
  ...
  if (plan.recharge?.packsUsed?.length > 0) {
    ...
    autoTable(doc, {
      ...
      body: plan.recharge.packsUsed.map((p) => [
        ...
        p.type,
        `x${p.count}`,
        (p.diaTotal ? p.dia : p.count * p.dia).toLocaleString(),
  ...
  // Day-by-day schedule
  const startDay = plan.startDay ?? 1;
  const allRows = plan.daySchedule?.rows ?? [];
  const rows = allRows.filter((r) => r.day >= startDay);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(11);
  doc.setTextColor(...COLORS.text);
  ...
  if (startDay > 1) {
    ...
    doc.setFontSize(8);
    doc.setTextColor(...COLORS.muted);
    doc.text(`Plan shown from Day ${startDay} onward.`, MARGIN, y);
    y += 12;
  } else {
    y += 6;
  }
  ...
  let cumulative = 0;
  const isAspirants = !isCollector && event?.id === "aspirants_2026";
  const head = isCollector ? ["Date", "Action", "Draws", "Total", "Dia", "CoA"] : ["Date", "Action", "Draws", "Total", "Dia"];

  const body = [];
  const rowNotesLookup = [];
  const rowDayLookup = []; // day for each body row (for didParseCell styling)
  const rowIsSubrow = []; // whether this body row is a subrow
  for (const r of rows) {
    const cumulativeBefore = cumulative;
    // The planner publishes the first-time-10x day's two accounting units as
    // r.main / r.subrow (see planOrchestrator.attachAspirantsSubrowSplit): the
    // PDF prints the main row with its OWN recharge/spend and the sub row with
    // the 10x + the packs bought that day. Note-matching is the fallback for
    // plans built before that field existed.
    ...
    if (isAspirants && (planSub || r.day === plan.checkpointDay)) {
      ...
      if (tenxNotes.length > 0) {
        ...
        const dailyDraws = planMain?.draws ?? (dailyNotes.length > 0 ? Math.max(0, r.draws - tenxDraws) : 0);
        const dailyDia = planMain?.diaSpent ?? (dailyNotes.length > 0 ? Math.max(0, (r.diaSpent ?? 0) - tenxDiaVal) : 0);
        const sanitizedDailyNotes = dailyNotes.map(sanitizePdfText);
        const dailyAction = sanitizedDailyNotes.join("\n") || sanitizePdfText("1x daily");
        const dailyCumulative = cumulativeBefore + dailyDraws;
        const dailyBase = [dayToDate(event.start_date, r.day), dailyAction, dailyDraws, dailyCumulative, dailyDia > 0 ? dailyDia.toLocaleString() : DASH];
        if (isCollector) dailyBase.push(DASH);
        body.push(dailyBase);
        rowNotesLookup.push(sanitizedDailyNotes);
        rowDayLookup.push(r.day);
        rowIsSubrow.push(false);
        const sanitizedTenxNotes = tenxNotes.map(sanitizePdfText);
        const tenxAction = sanitizedTenxNotes.join("\n");
        const tenxCumulative = dailyCumulative + tenxDraws;
        cumulative = tenxCumulative;
        const subrowBase = [dayToDate(event.start_date, r.day), `If no bingo at 40: ${tenxAction}`, tenxDraws, tenxCumulative, tenxDiaVal > 0 ? tenxDiaVal.toLocaleString() : DASH];
        if (isCollector) subrowBase.push(DASH);
        body.push(subrowBase);
        rowNotesLookup.push(sanitizedTenxNotes);
        rowDayLookup.push(r.day);
        rowIsSubrow.push(true);
        continue;
      }
    }
    cumulative += r.draws;
    ...
    const base = [dayToDate(event.start_date, r.day), action, r.draws, cumulative, r.diaSpent > 0 ? r.diaSpent.toLocaleString() : DASH];
    if (isCollector) base.push(r.coaSpent > 0 ? r.coaSpent.toLocaleString() : DASH);
    body.push(base);
    rowNotesLookup.push(sanitizedNotes);
    rowDayLookup.push(r.day);
    rowIsSubrow.push(false);
  }

  // Totals row — summed from displayed (filtered) rows only
  const totalDraws = rows.reduce((s, r) => s + r.draws, 0);
  const totalDia = rows.reduce((s, r) => s + (r.diaSpent ?? 0), 0);
  const totalCoa = rows.reduce((s, r) => s + (r.coaSpent ?? 0), 0);
  const footRow = isCollector
    ? [{ content: "Total from Day " + startDay, colSpan: 2 }, totalDraws, cumulative, totalDia.toLocaleString(), totalCoa.toLocaleString()]
    : [{ content: "Total from Day " + startDay, colSpan: 2 }, totalDraws, cumulative, totalDia.toLocaleString()];
  ...
  autoTable(doc, {
    ...
    didParseCell: (data) => {
      ...
      const day = rowDayLookup[data.row.index];
      const isSubrow = rowIsSubrow[data.row.index];
      const isFinal = day === duration;
      const notes = rowNotesLookup[data.row.index] ?? [];
      if (isSubrow) {
        data.cell.styles.fillColor = [255, 249, 230]; // light gold for subrow
      } else if (isFinal) {
        data.cell.styles.fillColor = COLORS.finalTint;
...
function buildBingoPdf(doc, plan, event) {
  const bingoLabel = event.id === "aspirants_2026" ? "The Aspirants" : `Bingo ${DASH} first line completion`;
  let y = addHeader(doc, event, bingoLabel);

  const wc = plan.winCondition;
  const bdt = wc.bdtCost;
  const fmtTk = (v) => `${TK} ${(v ?? 0).toLocaleString()}`;
  doc.setFont("helvetica", "bold");
  doc.setFontSize(11);
  doc.setTextColor(...COLORS.text);
  ...
  autoTable(doc, {
    startY: y,
    margin: { left: MARGIN, right: MARGIN },
    head: [["Scenario", "Draws", "Diamond Cost", "Plan Cost (BDT)"]],
    body: [
      ["Lucky", `${wc.draws.lucky[0]}${DASH}${wc.draws.lucky[1]}`, `${wc.diamondCost.lucky[0].toLocaleString()}${DASH}${wc.diamondCost.lucky[1].toLocaleString()}`, bdt ? `${fmtTk(bdt.lucky[0])}${DASH}${fmtTk(bdt.lucky[1])}` : fmtTk(0)],
      ["Realistic", `${wc.draws.realistic}`, wc.diamondCost.realistic.toLocaleString(), bdt ? fmtTk(bdt.realistic) : fmtTk(0)],
      ["Worst case", `${wc.draws.worst}`, wc.diamondCost.worst.toLocaleString(), bdt ? fmtTk(bdt.worst) : fmtTk(0)],
    ],
    headStyles: { fillColor: COLORS.goldLight, textColor: COLORS.text, fontStyle: "bold" },
