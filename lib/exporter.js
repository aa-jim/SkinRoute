...
function buildStandardPdf(doc, plan, event) {
  ...
  const hasOutfit1Header = plan.target.outfit1 && plan.target.skin.outfit1_variant;
  const targetLabel = hasOutfit1Header
  ...
  const hasOutfit1 = plan.target.outfit1 && plan.target.skin.outfit1_variant;
  const cardList = [
