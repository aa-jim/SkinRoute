...
export const REALISTIC_MULTIPLIER = 0.95;
export const WORST_CASE_MULTIPLIER = 0.85;
...
// UPDATED 2026-07-19: no longer read internally by drawsNeeded() — kept exported only for
// backward compat / display purposes. drawsNeeded() now takes a real `confidence` param
// (default "optimistic", matching this constant's old value) so each of the 3 confidence
// tabs (Optimistic/Realistic/Worst Case) shows its own distinct draws_needed figure instead
// of always showing the optimistic number regardless of which tab is selected.
 ...
 * UPDATED 2026-07-19: confidence is now a real parameter (was previously hardcoded to
 * read the module-level DRAWS_NEEDED_DIVISOR constant, so every caller got the same
 * "optimistic" figure regardless of which confidence tab the UI had selected). Default
 * stays "optimistic" for any existing caller that doesn't pass this arg — no behavior
 * change unless the caller explicitly passes "realistic" or "worst".
 *
 ...
 * @param {Confidence} confidence - "optimistic" | "realistic" | "worst" (default "optimistic")
 * @returns {{draws:number, gapAfterMilestones:number, milestonesApplied:object[], effectiveE:number}}
 */
export function drawsNeeded(event, targetCrests, currentCrests = 0, ownedGroups = {}, ownedSkinIds = [], confidence = "optimistic") {
...
export function bingoWinCondition(event, freeTokens = 0, ownedSkinIds = []) {
  ...
  function costFor(totalDraws) {
    ...
    if (remaining > 0) {
      ...
      cost += tenxBatches * cost10x;
      remaining -= tenxBatches * 10;
      cost += remaining * cost1xFull;
    }
    return cost;
  }
