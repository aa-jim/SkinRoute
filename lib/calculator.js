...
export function netDiamondsNeeded(
  ...
  if (gapAfterFree <= 0) {
    return { freeDrawsTotal, freeCrests, gapAfterFree: 0, extraDrawsNeeded: 0, netDiamondsNeeded: 0 };
  }

  const extraDrawsNeeded = Math.ceil(gapAfterFree / effectiveE);
  const netDia = extraDrawsNeeded * drawCost1x; // scheduler (5D) decides bulk/daily-discount mix

  return { freeDrawsTotal, freeCrests, gapAfterFree, extraDrawsNeeded, netDiamondsNeeded: netDia };
}
