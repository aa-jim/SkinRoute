...
function buildBingoPlanForTarget(event, resources, ownedItems, confidence, overrideStartDay, targetDrawsOverride, isProbe) {
    ...
    // packs + a shortfall top-up via the same knapsack the day-21/31 fix blocks
    // use). NOTE: passes bought the same day drip SEQUENTIALLY, so N phase-start
    ...
    // adopted when it costs at most one pass's price (wp.bdt) more than keeping
    ...
    if (isAspirants && extraPasses > 0 && !passesInBalance
        ...
        // coverable by roughly one pass's worth of packs. At low diamond
        // balances the day-1 pass IS the cheapest dia source (passes ~1.1
        // dia/BDT vs regular packs ~0.55) - dropping it would need far more
        // than a pass's price in top-up packs, so it stays.
        if (best && keep && best.bdt <= keep.bdt + wpBdt && best.topUpBdt <= wpBdt + 10) {
            dropFirstPass = best.dropMode;
            optimalPassCount = best.totalPasses;
            extraPasses = optimalPassCount - passCount;
            injectPhasePacks(); // commit the winner's packs; any residual shortfall is closed by the day-21/31 blocks below
        }
    }
    ...
    if (isAspirants && sim.unaffordableDiaShortfall > 0) {
        if (startDay <= 20) {
            // The first-time 10x lands on Day 20 and earns draws 41–50 - it is
            // only worth funding when the target needs it (targets > 40). Tier
            // probes at 30/40 draws skip this so the 10x pack is not bought
            // for draws the capping logic will refund anyway.
            if (targetDraws > 40) {
    ...
    if (extraPasses > 0) {
        if (isAspirants) {
            ...
            const totalP = distributePassPurchases(optimalPassCount, firstPassBuyDay, phaseStarts, dropFirstPass);
            ...
            for (const [dayStr, count] of Object.entries(extraByDay)) {
                if (count > 0) {
                    ...
                    // A pass "bought" before the plan window is still paid while
                    // following the schedule — pin it to the first planable day.
    ...
    if (isAspirants) {
        const base = { startingDia, passCount: optimalPassCount, firstPassBuyDay, passesInBalance, startDay, dropFirstPass, passPhaseStarts: remainingPhaseStarts };
