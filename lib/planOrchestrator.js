...
function buildBingoPlanForTarget(event, resources, ownedItems, confidence, overrideStartDay, targetDrawsOverride, isProbe) {
    ...
    if (isAspirants) {
        ...
        const firstAccDay = Math.min(Math.max(startDay, aspirantsLastPhaseEnd + 1), event.duration_days);
        ...
        for (let guard = 0; guard < 8 && sim.unaffordableDiaShortfall > 0; guard++) {
            let cum = 0, checkpointDay = 0;
            for (const r of sim.rows) {
                cum += r.draws;
                if (cum >= 40) { checkpointDay = r.day; break; }
            }
            ...
            finalCheckpointDay = checkpointDay;
