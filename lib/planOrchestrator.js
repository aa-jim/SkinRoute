...
function buildThemedCrestPlan(event, resources, target, ownedItems, confidence) {
    ...
    function addPacksToDay(day, needs) {
        const fix = optimizeRecharge(needs, { regular: packsData.regular, first_purchase: packsData.first_purchase }, event.duration_days, { fpClaimed: usedFpClaimed });
        if (fix.packsUsed && !fix.impossible) {
    ...
    for (const phase of supply) {
        ...
        const existingRecharge = extraDiaByDay[phase.start_day] || 0;
        ...
        if (totalNeed > 0) {
            addPacksToDay(phase.start_day, totalNeed);
        }
    }
