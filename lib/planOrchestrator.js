...
function buildBingoPlanForTarget(event, resources, ownedItems, confidence, overrideStartDay, targetDrawsOverride, isProbe) {
    ...
    if (isAspirants) {
        ...
        for (let guard = 0; guard < 8 && sim.unaffordableDiaShortfall > 0; guard++) {
            ...
            // Whether THIS iteration injected diamonds. Tracked because the deficit
            // is NOT monotone in the injected amount: a top-up re-times the organic
            // 40-checkpoint / first-time 10x and can RAISE the reported shortfall
            // (observed day-1 0-dia trace: 3073 -> 3898 -> 1645 -> 175). When a
            // branch decides there is "nothing to do", the loop must still
            // reconcile the recomputed residual instead of burning the guard budget
            // and exiting with an unfunded plan.
            let injected = false;
        ...
        if (finalCheckpointDay === 0 || finalTenxDay === 0) {
            ...
            const firstAccDay2 = firstAccDayOf(sim);
