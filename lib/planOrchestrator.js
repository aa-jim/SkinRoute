...
function buildThemedCrestPlan(event, resources, target, ownedItems, confidence) {
    ...
    // ---- Real day-by-day simulation, pass-search sized (mirrors Collector's approach) ----
    // Passes bought per BUILD_PLAN.md's "split across Day 1 + each supply window start
    // day" rule (buildNaivePassPurchases) — sized by duration first, then topped up
    // specifically at each premium supply window if the base count isn't enough there.
    function simulateWithPassCount(passCount) {
        ...
        const daySchedule = buildThemedCrestDaySchedule(
            event, resources, passPlan.dailyDiamondSchedule, draws.draws, milestones
        );
        return { passPlan, daySchedule };
    }
    ...
    const { passPlan, daySchedule } = sim;
    ...
    return {
        ...
        recommendedPasses,
        rawPassCount,
        warnings: buildThemedCrestWarnings(idealPlan, netDia, recommendedPasses, rawPassCount),
    };
}
...
function buildThemedCrestWarnings(idealPlan, netDia, recommendedPasses, rawPassCount) {
    const warnings = [];
    ...
    if (recommendedPasses > rawPassCount) {
        ...
        warnings.push({
            ...
            message: `Current resources can't complete this plan — buy ${extra} more weekly pass${extra > 1 ? "es" : ""} (see Recommended Recharge below) to reach the target.`,
        });
    } else if (netDia.netDiamondsNeeded > 0) {
        warnings.push({
...
// ---------------------------------------------------------------------------
// Themed Crest / Legend / Special — real day-by-day schedule (Part 2)
// Diamonds-only mirror of buildCollectorDaySchedule(), built incrementally:
//   2a (this pass): daily 1x every day + final-day gap-close + unaffordableDiaShortfall
//   2b: premium supply window recharge/spend token granting
//   2c: milestone token granting + surprise ladder note
// ---------------------------------------------------------------------------

/**
 * Diamonds-only day-by-day walk for Themed Crest / Legend / Special events.
 * Same shape/pattern as buildCollectorDaySchedule() (rows + totals +
 * unaffordableDiaShortfall) but no CoA, no Starlight — just diamonds.
 *
 * @param {object} event
 ...
 * @param {number} totalDrawsTarget - from drawsNeeded().draws
 * @returns {{
 *   rows: Array<{day:number, draws:number, diaSpent:number, notes:string[]}>,
 *   totals: {draws:number, dia:number},
 *   unaffordableDiaShortfall: number
 * }}
 */
function buildThemedCrestDaySchedule(event, resources, dailyDiamondSchedule, totalDrawsTarget, milestones = []) {
    ...
    for (let day = 1; day <= duration; day++) {
        rows.push({ day, draws: 0, diaSpent: 0, notes: [] });
    }
    ...
    // Premium supply: precompute each phase's target spend amount (cheaper of daily+
    // singles vs 10x-bulk, per spendTaskStrategy) for its highest spend_* tier, then
    // track cumulative window spend/recharge day-by-day to grant tokens as thresholds
    // cross — same first-crossing pattern used for Collector's spend-task fix.
    const supplyPhases = Array.isArray(event.premium_supply) ? event.premium_supply : [];
    const phaseState = supplyPhases.map((phase) => {
        const windowEndDay = phase.start_day + phase.duration_days - 1;
        const spendTasks = (phase.tasks ?? []).filter((t) => t.type?.startsWith("spend_"));
        const highestSpendTask = spendTasks.reduce(
            (best, t) => (t.threshold_dia > (best?.threshold_dia ?? -1) ? t : best), null
        );
        const strategy = highestSpendTask
            ? spendTaskStrategy(event, highestSpendTask.threshold_dia, phase.duration_days)
            : null;
        const targetWindowSpend = strategy
            ? Math.min(strategy.pathA.diamonds, strategy.pathB.diamonds)
            : 0;
        return {
            phase,
            windowStartDay: phase.start_day,
            windowEndDay,
            spendTasks: spendTasks.sort((a, b) => a.threshold_dia - b.threshold_dia),
            loginTask: (phase.tasks ?? []).find((t) => t.type === "login"),
            rechargeTasks: (phase.tasks ?? []).filter((t) => t.type?.startsWith("recharge_")),
            targetWindowSpend,
            cumulativeWindowSpend: 0,
            spendTiersClaimed: new Set(),
            rechargeTiersClaimed: new Set(),
            loginClaimed: false,
        };
    });
    ...
    for (let day = 1; day <= duration; day++) {
        ...
        diaBalance += dailyDiamondSchedule?.[day] ?? 0;
        ...
        // Mandatory discounted daily 1x — every day including the final one (Collector
        // bug #5 lesson applied here from the start: never wrap this in !isFinalDay).
        if (diaBalance >= dailyCost) {
            diaBalance -= dailyCost;
            row(day).diaSpent += dailyCost;
            row(day).draws += 1;
        ...
        // Premium supply window: top up with regular singles toward the phase's target
        // window spend (cheaper-path amount from spendTaskStrategy, precomputed above),
        // then grant login/spend/recharge tokens the day their threshold is first crossed.
        const activePhase = phaseState.find((ps) => day >= ps.windowStartDay && day <= ps.windowEndDay);
        if (activePhase && !isFinalDay) {
            if (activePhase.loginTask && !activePhase.loginClaimed && day === activePhase.windowStartDay) {
                activePhase.loginClaimed = true;
                const t = activePhase.loginTask.tokens ?? 0;
                row(day).notes.push(`Claim ${t} key (login task) \u2192 ${t} free draw${t > 1 ? "s" : ""}`);
                row(day).draws += t;
                drawsSoFar += t;
            }

            const gapToTarget = activePhase.targetWindowSpend - activePhase.cumulativeWindowSpend;
            if (gapToTarget > 0 && diaBalance >= fullCost) {
                // Top up with minimum singles to reach the target window spend, capped by
                // what's actually affordable this day — never a 10x bulk purely for spend
                // (per spendTaskStrategy's own confirmed technique).
                const singlesAffordable = Math.floor(diaBalance / fullCost);
                const singlesNeeded = Math.min(singlesAffordable, Math.ceil(gapToTarget / fullCost));
                if (singlesNeeded > 0) {
                    diaBalance -= singlesNeeded * fullCost;
                    row(day).diaSpent += singlesNeeded * fullCost;
                    row(day).draws += singlesNeeded;
                    row(day).notes.push(`${singlesNeeded}x top-up single (spend task)`);
                    drawsSoFar += singlesNeeded;
                }
            }
            activePhase.cumulativeWindowSpend += row(day).diaSpent;

            for (const tier of activePhase.spendTasks) {
                if (!activePhase.spendTiersClaimed.has(tier.type) && activePhase.cumulativeWindowSpend >= tier.threshold_dia) {
                    activePhase.spendTiersClaimed.add(tier.type);
            ...
            for (const tier of activePhase.rechargeTasks) {
                const threshold = tier.threshold_dia ?? 0;
                if (!activePhase.rechargeTiersClaimed.has(tier.type) && (dailyDiamondSchedule?.[day] ?? 0) > 0) {
                    // Recharge tokens trigger off cumulative diamonds RECHARGED (pass drip
                    // landing), not spent — approximate via cumulative pass inflow this window.
                    const cumulativeRecharged = Object.entries(dailyDiamondSchedule ?? {})
                        .filter(([d]) => Number(d) >= activePhase.windowStartDay && Number(d) <= day)
                        .reduce((sum, [, v]) => sum + v, 0);
                    if (cumulativeRecharged >= threshold) {
                        activePhase.rechargeTiersClaimed.add(tier.type);
                        const t = tier.tokens ?? 0;
                        row(day).notes.push(`Claim ${t} key${t > 1 ? "s" : ""} (recharge ${threshold} dia task) \u2192 ${t} free draw${t > 1 ? "s" : ""}`);
                        row(day).draws += t;
                        drawsSoFar += t;
                    }
                }
        ...
        // Milestone tokens: grant the first day the real simulated drawsSoFar crosses
        // each threshold — same real-time crossing pattern as the Collector fix (not a
        // fixed approxDay guess). Crest-type milestones are already subtracted from the
        // target by drawsNeeded() upstream — only token-type milestones grant free draws here.
        for (const m of milestones) {
        ...
        if (isFinalDay) {
            ...
            if (remainingDraws > 0) {
                const diaCost = remainingDraws >= 10
                    ? Math.floor(remainingDraws / 10) * cost10x + (remainingDraws % 10) * fullCost
                    : remainingDraws * fullCost;
                diaBalance -= diaCost;
                row(day).diaSpent += diaCost;
                row(day).draws += remainingDraws;
                row(day).notes.push(remainingDraws >= 10 ? `${remainingDraws}x final push (dia, 10x draw)` : `${remainingDraws}x final push (dia)`);
                if (diaBalance < 0) {
            ...
            } else if (row(day).draws === 0) {
                row(day).notes.push("No further draws needed \u2014 target already reached");
            }
        }
    }
    ...
    // Surprise ladder (informational only — Street Fighter-style cumulative recharge
    // tiers, most give items/crates not draw tokens; see surpriseTaskProgress()).
    const totalRecharged = Object.values(dailyDiamondSchedule ?? {}).reduce((a, b) => a + b, 0)
        + Number(resources?.diamonds ?? 0);
    const surpriseLadder = surpriseTaskProgress(event, totalRecharged);

    return { rows, totals, unaffordableDiaShortfall, surpriseLadder };
}
