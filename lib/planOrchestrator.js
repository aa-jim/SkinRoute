...
function buildBingoPlan(event, resources, ownedItems, confidence, overrideStartDay) {
    ...
    if (isAspirants) {
        const base = { startingDia, passCount: optimalPassCount, firstPassBuyDay, passesInBalance, startDay };
    ...
    // Per-tier BDT cost (all bingo events): sum the actual pack + pass purchases
    // the one day-by-day schedule makes on or before the day each draw tier is
    // reached. This is a strict prefix of the single full-event recharge plan —
    // the money is computed ONCE in `recharge`; this only reads *where it is
    // spent* from the schedule the user is shown. worst always equals
    // recharge.totalBdt. 0 means that tier is covered by owned diamonds only.
    function computeBingoWinConditionBdt() {
        const passBdt = packsData.weekly_pass?.bdt ?? 0;
        const bdtByDay = new Map();
        for (const [dayStr, packs] of packsByDay) {
            const day = Number(dayStr);
            const sum = packs.reduce((s, p) => s + p.count * p.bdt, 0);
            bdtByDay.set(day, (bdtByDay.get(day) ?? 0) + sum);
        }
        for (const r of passReceipts) {
            const day = Math.max(1, r.day);
            bdtByDay.set(day, (bdtByDay.get(day) ?? 0) + r.count * passBdt);
        }
        const bdtForTarget = (T) => {
            let spent = 0;
            let cumDraws = 0;
            for (const row of sim.rows) {
                cumDraws += row.draws;
                spent += bdtByDay.get(row.day) ?? 0;
                if (cumDraws >= T) return spent;
            }
            return spent;
        };
        const d = winCondition.draws;
        return {
            lucky: [bdtForTarget(d.lucky[0]), bdtForTarget(d.lucky[1])],
            realistic: bdtForTarget(d.realistic),
            worst: bdtForTarget(d.worst),
        };
    }
    winCondition.bdtCost = computeBingoWinConditionBdt();
