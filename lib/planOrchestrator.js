...
/**
 * Convert the user's firstPassDate string (e.g. "2026-06-28") to an event-relative
 * day number. A date before event start yields a negative day (pass drip already
 * partially consumed before event). Defaults to 1 (first pass bought on event day 1).
 */
function passFirstBuyDay(resources, event) {
    const dateStr = resources?.firstPassDate;
    if (!dateStr || !event.start_date) return 1;
    const diff = (new Date(dateStr) - new Date(event.start_date)) / (1000 * 60 * 60 * 24);
...
function buildCollectorPlan(event, resources, target, ownedItems, confidence) {
    ...
    if (sim.day1RechargeNeeded > 0) {
        const fix = optimizeRecharge(sim.day1RechargeNeeded, { regular: packsData.regular }, event.duration_days, resources);
    ...
    if (sim.unaffordableDiaShortfall > 0) {
        const finalDayFix = optimizeRecharge(sim.unaffordableDiaShortfall, { regular: packsData.regular }, event.duration_days, resources);
    ...
    if (day1Packs.length > 0 || extraPasses > 0) {
        ...
        for (const e of day1Packs) {
            const name = e.id.replace(/^r_/, '');
...
function buildThemedCrestPlan(event, resources, target, ownedItems, confidence) {
    ...
    // Pass search: try higher pass counts one at a time, stop at first that closes
    // BOTH day-1 and final-day gaps to zero. Only counts up to max useful passes are
    // tried — passes beyond that don't realize full drip value before event end.
    const totalSpan = event.duration_days - firstPassBuyDay + 1;
    const maxUsefulPasses = Math.min(Math.max(Math.floor(totalSpan / 7), 0), 10);
    const searchEnd = Math.max(passCount, maxUsefulPasses);
    ...
    // Re-run sim with chosen pass count, then fill remaining gaps with packs
    ...
    // Re-run sim with day-1 pack diamonds
    sim = buildThemedCrestPlanWithPacks(event, packsData, draws.draws, {
        startingDia, passCount: optimalPassCount, firstPassBuyDay, passesInBalance,
    }, day1PackDia, 0);
    ...
    const milestones = milestoneTokenSchedule(event, draws);
    const totalDiamondsForPlan = startingDia + recharge.totalDia;
    // StepFour.jsx reads p.netDiamondsNeeded?.netDiamondsNeeded as the "extra needed"
    // sublabel — that field belonged to calculator.js's netDiamondsNeeded() output, which
    // this rewrite no longer calls (the simulator's own unaffordableDiaShortfall + recharge
    // sizing supersede it). Providing an equivalent shape here so the UI keeps working
    // without a StepFour.jsx edit — value is simply how many diamonds the recharge plan adds.
