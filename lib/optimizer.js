 ...
 * @param {boolean} rechargeMode - when true, FP pack weight = dia_base (bonus/extra don't count for recharge)
 * @returns {Array<{id:string, weight:number, realDia:number, cost:number, once:boolean, itemTime:number, type:string}>}
 */
function buildItemList(packsData, fpClaimed = {}, rechargeMode = false) {
  const items = [];

  for (const pack of packsData.regular ?? []) {
    items.push({ id: pack.id, weight: pack.total, realDia: pack.total, cost: pack.bdt, once: false, itemTime: 0, type: "regular" });
  }
  ...
  if (pass) {
    ...
    items.push({ id: "weekly_pass", weight: passWeight, realDia: passWeight, cost: pass.bdt, once: false, itemTime: pass.days, type: "pass" });
  }

  for (const fp of packsData.first_purchase ?? []) {
    if (fpClaimed[fp.id]) continue; // already used — not buyable again
    // In recharge mode, only base dia counts toward the threshold (bonus/extra don't)
    const weight = rechargeMode ? fp.dia_base : fp.total;
    items.push({ id: fp.id, weight, realDia: fp.total, cost: fp.bdt, once: true, itemTime: 0, type: "fp" });
  }
...
function knapsack3D(container, maxDays, items) {
  ...
  for (let j = 0; j < n; j++) {
    if (count[j] > 0) {
      packsUsed.push({ id: items[j].id, type: items[j].type, count: count[j], bdt: items[j].cost, dia: items[j].realDia });
    }
  }
 ...
 * }}
 */
export function optimizeRecharge(containerDia, packsData, eventDurationDays, resources = {}, rechargeMode = false) {
  if (containerDia <= 0) {
    return { totalBdt: 0, totalDia: 0, completionDay: 0, packsUsed: [], impossible: false };
  }

  const items = buildItemList(packsData, resources.fpClaimed ?? {}, rechargeMode);
