  ...
  for (let day = startDay; day <= duration; day++) {
    ...
    if (accumulationMode) {
      ...
      if (useTenx) {
        ...
        // Lazy cadence: an accumulation day (a between-phase gap, or the calm
        // tail of a phase whose spend tasks are already cleared) skips its
        // discounted daily when drawing it would outrun the calendar:
        //
        // (a) CHECKPOINT ANCHOR - the 40th draw may not land before the first
        //     accumulation day. The checkpoint + same-day 10x belong right
        //     after the last supply phase (Sep 30 for a day-1 start), not
        //     mid-phase-2 at cum 42.
        // (b) CONVERGE ON THE CHECKPOINT - before the first accumulation day,
        //     today only matters if the calendar still arrives short without
        //     it. Every day up to and including the checkpoint day can draw
        //     one discounted daily and the phases still ahead hand over their
        //     free tokens by then, so when that future supply already carries
        //     the running total to the checkpoint the gap day has nothing to
        //     add - holding keeps the mid-gap dailies off the schedule (Sep 24
        //     on a day-1 start) and stops the double gap right in front of the
        //     checkpoint (Sep 28 + 29). The deficit, when one exists, lands on
        //     the LAST day before the checkpoint: same cost (one discounted
        //     daily either way), one unbroken cadence.
        // (c) SPARE CAPACITY - from the checkpoint day onward: if the remaining
        //     calendar (one discounted daily per day + the free draws future
        //     phase windows hand out + the reserved 10x) already covers the
        //     target, today's draw is stockpiling - hold it so the target is
        //     reached exactly on the final day instead of idling after.
        //
        ...
        const anchorHold = checkpointTarget > 0
          && totalDraws + 1 >= checkpointTarget && day < firstAccDay;
        let futureTokens = 0;
        for (const p of phaseList) {
          if (p.startDay > day) futureTokens += p.loginTokens + p.rechargeTotalTokens + p.spendTokens;
        }
        const convergeArrival = totalDraws + (firstAccDay - day) + futureTokens;
        ...
        const convergeHold = day < firstAccDay
          && convergeArrival >= checkpointTarget
        ...
        const cadenceHold = day < firstAccDay ? convergeHold : spareHold;
        if (anchorHold || (cadenceHold && canResume)) {
