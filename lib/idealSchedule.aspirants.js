  ...
  for (let day = startDay; day <= duration; day++) {
    ...
    if (accumulationMode) {
      ...
      if (useTenx) {
        ...
        // (b) SPARE CAPACITY - if the remaining calendar (one discounted
        //     daily per day + the free draws future phase windows hand out +
        //     the reserved 10x) already covers the target, today's draw is
        //     stockpiling - hold it so tiers land one per day and the target
        //     is reached exactly on the final day instead of idling after.
        //
        // Both holds are funding-gated: skipping today must not strand the
        ...
        if (anchorHold || (spareHold && canResume)) {
