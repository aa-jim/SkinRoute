...
- **Aspirants**: pre-phase days idle; in-phase daily 1x with spend-task keys per threshold; phase-start recharge claims; phase-close single top-up; after the last phase, accumulation mode with **lazy-cadence holds** (an accumulation day skips its discounted daily when drawing would cross 40 before the first accumulation day or the remaining calendar already covers the target — funding-gated so poor plans never change shape; first-time 10x only on/after the first accumulation day); final push priced at the discounted `daily_1x` per remaining draw.
