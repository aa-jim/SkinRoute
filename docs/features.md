...
- **Recommended Recharge** (`PackRecommendation`):
  ...
  - Aspirants (bingo): the pass search's first batch (1 pass on the plan's first day, before any supply phase) exists only to fund draws with dia. Candidate structures WITHOUT that unit are priced against the keep-plan — same pass count redistributed onto the remaining phase starts, or one fewer pass — including a small top-up pack when the sequential pass drip leaves a residual gap (e.g. dia 2,025 + 3 phase passes + the phase-2 pack is really ~2,840 in-window, not 2,940). The cheapest candidate that costs at most 30 BDT (~15% of a pass) more than keeping is adopted, so the **day-1 "Buy 1× weekly pass" line disappears** (e.g. 3× day 3 instead of 1× day 1 + 3× day 8) — the re-planned candidate usually undercuts the keep-plan outright (dia 2,025 → ৳1,070 with 3× phase-start passes + r_257; dia 0 → ৳3,042). Phase-start (recharge-task) passes are never dropped; mid-event plans keep the previous structure, and passes are only swapped for packs when the price says so. For bingo, each tier's BDT line is priced by planning for that tier (its own pass search/packs at that target; the worst tier stays the full plan), so a 50-draw plan at 0 dia reads ~Tk 2,938 instead of the worst-case prefix (Tk 4,934); tiers are capped monotonic. Draw pacing is checkpoint-staged: the simulator reserves the first-time 10x for the 40→50 hop (it only fires once 40 draws are reached and the target needs draws 41+), a starved day gets a small top-up pack on that day so the 1/day cadence lands on 40 as early as the calendar allows, the 10x fires **on the checkpoint day itself** (the same-day rule: 40 reached and no bingo → buy for 50 and take the 10x immediately, so the next day keeps its daily draw instead of wasting it — e.g. Oct 1 = 1 daily + 10 10x = draws 40 *and* 50, Oct 2 still draws its daily), the tail money lands on the day after, and the final push notes sequence 40 → 50 → 60 so the schedule always lands exactly on each win tier.
