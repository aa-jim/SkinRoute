...
- **Recommended Recharge** (`PackRecommendation`):
  ...
  - Aspirants (bingo): the pass search's first batch (1 pass on the plan's first day, before any supply phase) exists only to fund draws with dia. Candidate structures WITHOUT that unit are priced against the keep-plan — same pass count redistributed onto the remaining phase starts, or one fewer pass — including a small top-up pack when the sequential pass drip leaves a residual gap (e.g. dia 2,025 + 3 phase passes + the phase-2 pack is really ~2,840 in-window, not 2,940). The cheapest candidate that costs at most 30 BDT (~15% of a pass) more than keeping is adopted, so the **day-1 "Buy 1× weekly pass" line disappears** (e.g. 3× day 3 instead of 1× day 1 + 3× day 8) — the re-planned candidate usually undercuts the keep-plan outright (dia 2,025 → ৳1,070 with 3× phase-start passes + r_257; dia 0 → ৳3,415 with the first-purchase bonuses, ৳5,141 without). Phase-start (recharge-task) passes are never dropped; mid-event plans keep the previous structure, and passes are only swapped for packs when the price says so. For bingo, each tier's BDT line is priced by planning for that tier (its own pass search/packs at that target; the worst tier stays the full plan), so a 50-draw plan at 0 dia reads ~Tk 2,395 instead of the worst-case prefix (Tk 4,934); tiers are capped monotonic. Draw pacing is checkpoint-staged: the simulator reserves the first-time 10x for the 40→50 hop (it only fires once 40 draws are reached and the target needs draws 41+), a starved day gets a small top-up pack on that day so the 1/day cadence lands on 40 as early as the calendar allows, the 50-push packs are dated on the day after the checkpoint (the day the 10x fires — never on the checkpoint day itself), and the final push notes sequence 40 → 50 → 60 so the schedule always lands exactly on each win tier. **Every plan is fully funded** — after each staged injection the loop reconciles the recomputed residual, and when the tail heuristic's "50%-off daily on every remaining day" estimate would swallow a real gap it funds the raw shortfall instead (before this fix the plan could ship 175–734 diamonds short with an "N extra diamonds needed" warning while simultaneously over-buying — e.g. an `r_2195` left 4,816 dia idle and still warned). The engine's worst-case money at 0 dia / day 1 is **1,070** (30 and 40 draws), ৳2,395 (50), **৳3,415** (60) with the first-purchase bonuses available, or ৳5,141 when all four are already claimed; `npm run verify:aspirants` asserts 60 draws, 0 warnings and full funding across the whole diamonds × start-day grid.
