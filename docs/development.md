...
Primary deployment is Cloudflare (`https://skinroute.events-mlbb.workers.dev/`, free plan) — Vercel (`skin-route.vercel.app`) is kept in parallel as a fallback. Stack: `@opennextjs/cloudflare` adapter runs the Next.js app on the Workers runtime (no Node server; Node.js APIs via `nodejs_compat`). Note: the Workers **free plan caps at 100k requests/day account-wide** — a viral day (FB group post, ~138k requests) will throttle/briefly error the site past the cap; that is capacity, not an attack (see Traffic management below).
...
- **Static caching**: `public/_headers` serves `Cache-Control: public,max-age=31536000,immutable` for `/_next/static/*` (fingerprinted files) and `public,max-age=604800` (7 days) for `/assets/*` (event banners/skins are NOT content-hashed, so a 7-day TTL lets swaps propagate). Vercel ignores this file.

## Traffic management (free plan)

The site survived a viral Facebook-group day (~138k requests/24h, ~20k Worker invocations). What keeps a repeat of that inside the free tier:

- **HTML is CDN-cached** — `next.config.mjs` sets `Cache-Control: public, s-maxage=60, stale-while-revalidate=300` on `/`, `/help`, `/plan/*`. Repeat visitors (crawlers, re-shares) get an edge copy instead of re-rendering on the Worker; most page views never invoke the Worker. The cached response carries its own baked CSP nonce (consistent with its HTML, no POST forms on these pages → safe).
- **`POST /api/plan` is rate-limited in-worker** — sliding window per `cf-connecting-ip`, 30 req/60s → `429` + `Retry-After`. `buildPlan` is the only compute-heavy unauthenticated route, and Cloudflare's edge rate-limiting free budget (10k req/month) is far too small for this traffic, so the guard lives in the route handler (per-isolate map; a blunt guard, not hard accounting). Map is size-capped (10k IPs, swept).
- **AI/scraper crawlers are blocked twice**: `public/robots.txt` disallows GPTBot/ClaudeBot/CCBot/Amazonbot/Bytespider/PerplexityBot/meta-externalagent/Google-Extended/SEO bots etc., and `middleware.js` returns `403` for the same UA substrings before any render work. Search engines (Google/Bing) and Facebook's preview crawler are intentionally allowed.
- **Dashboard (free)**: Bot Fight Mode + Browser Integrity Check on; set a usage alert to be warned before the 100k/day cap. Cloudflare's dashboard Rate Limiting is NOT viable here (10k req/month budget).
