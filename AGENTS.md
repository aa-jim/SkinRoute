...
## Recent Changes
### Aspirants: no-gap ladder into the checkpoint — same-day 10x one day earlier (Sep 24 2026)
- **Problem (user report, day-1 / 2,000-dia fp-none)**: the converge-on-checkpoint hold skipped 9/28 too — a cum-38 day (already past the 30-draw tier) sitting dark right in front of the checkpoint, so 39 landed on 9/29 and 40 + the same-day 10x on 9/30. Target pacing: draw every day from the 30-draw tier on — 39 on 9/28, 40 + same-day 10x on 9/29.
- **Fix — token-guarded hold** (`lib/idealSchedule.aspirants.js`): `convergeHold` is now scoped `futureTokens > 0`. The hold's only job is to stop PENDING phase tokens from overshooting the checkpoint (the 9/21-24 holds keep the plateau at cum 19 while phase 2's 16 tokens are still bankable); once every token is claimed (the last phase's calm tail) it always draws, so the ladder runs 34 (9/25) → 35 → 38 → **39 (9/28)** → **40 + same-day 10x (9/29)** → 51 (9/30) → 60 on 10/8. An intermediate `isGapDay`-only scoping was tried first and rejected: it broke the 9/21-24 plateau, crossed 40 on 9/27 and left a double 9/28+9/29 gap while the 10x waited for `firstAccDay`.
- **Report (verified, 78-cell A/B vs HEAD)**: start-1/4/5 cells move checkpoint + 10x to 9/29, spend 105 dia less (one discounted daily replaces two full-price push singles) and are mostly cheaper: day-1 2,000 fp-none **৳1,380 → ৳1,236**, 2,025 → **৳1,175**, start-5 2,000 → **৳1,304**, fp-all day-1 → ৳1,294, 0-dia fp-all → ৳4,940; 0-dia fp-none keeps **৳3,415** (shape: checkpoint + 10x move d15 → d14, same packs one day earlier); day-500 cells pay +296..+359 (funding the 9/28 daily earlier — the surviving tier-pacing guard rejects the cheaper-but-slower candidate); starts 7/10/17/21/23 byte-identical. No zero-draw day after cum 30 among rich starts.
- **Verify**: `verify:aspirants` **PASS 144** (60 draws, 0 warnings, funded, tiers monotonic) + 6 byte-identical non-aspirants; checker 12 now asserts checkpoint day 14 + same-day 10x + no zero-draw day after the 30-draw tier; pin re-pinned `9e06d324be8a13ec` → `772ce2f03ad12bcf` with rationale (the ৳3,415 reference plan keeps its money, the 10x hop moves one day earlier); `next lint` (3 pre-existing `<img>` warnings) + `next build` pass.
- Docs: `docs/architecture.md` (new token-guard paragraph + simulator bullet), `docs/features.md` (draw-pacing + prices), `docs/aspirants2026_status.md` (§3 resolved block), pin/checker comments in `scripts/verify-aspirants.mjs`.

### Aspirants: converge-on-checkpoint pacing + pass-structure guard (Sep 22 2026)
- **Problem (user report, startDay-5 / 2,000-dia probes)**: the lazy-cadence `spareHold` modelled pre-checkpoint capacity as "remaining calendar days + future tokens", but the checkpoint anchor rule means the last days before `firstAccDay` can never draw — so the model over-counted capacity by exactly those days, killed cheap dailies mid-gap (a stockpile draw on 9/24 at cum 19→20) and left a double gap (9/28 + 9/29) right in front of the checkpoint, pushing full-price singles onto 10/8. Separately, the drop-candidate pass search only ran when `firstPassBuyDay < firstRemainingPhaseStart`, so mid-phase starts (d5 inside phase 1) never got the drop trial and kept a 4th pass (৳1,660); and an accidental `buildPassDripSchedule` change had flipped the pass drip to PARALLEL (3 same-day passes = 60/day), contradicting the game's sequential queue and inflating in-window dia (flagship dia-2,025 case drifted to 5 passes / ৳1,294).
- **Fix (1) — converge-on-checkpoint hold** (`lib/idealSchedule.aspirants.js`): for accumulation days before `firstAccDay`, `spareHold` is replaced by a **deficit rule**: compute `convergeArrival = totalDraws + daysLeftBeforeCheckpoint + futurePhaseTokens` — if the calendar arrives at the checkpoint at/above 40 WITHOUT today's draw, hold (the deficit, when one exists, lands on the LAST pre-checkpoint day, so 39 lands on 9/29 and the checkpoint day reads exactly 40 → same-day 10x → 50). A `payableDays` projection (balance + booked drip/injections over the remaining pre-checkpoint days) keeps the hold funding-gated: a balance-poor plan only rests on days it genuinely doesn't need, so 0-dia plans stay need-driven. `anchorHold` and the post-checkpoint `spareHold` (tail pacing) are unchanged.
- **Fix (2) — starved-day counter**: the old `else if (dia < dailyCost) {}` silently swallowed a wanted draw the balance couldn't pay for; the sim now counts `starvedDays` and the orchestrator's tier-pacing guard (below) rejects any candidate structure that starves days the keep-plan doesn't.
- **Fix (3) — pass-structure guard** (`lib/planOrchestrator.js`): the drop-candidate search now also runs for mid-phase starts (`firstPassBuyDay <= firstRemainingPhaseStart`) and every candidate must not pace worse than the keep-plan — `trial.preAccDraws >= keep.preAccDraws` (pre-checkpoint draw count) and `checkpointDay <= keep.checkpointDay` — so a cheaper-but-slower structure can never buy its price saving by slipping the win tiers. Result: startDay-5/2,000-dia drops to 3 passes / ৳1,380.
- **Fix (4) — sequential drip restored** (`buildPassDripSchedule`): `dripStart = Math.max(queueCursor, pp.day)` again (one pass runs its 7 days, then the next) — matches `data/packs.json` and `lib/scheduler.js`; parallel drip removed.
- **Report (verified)**: day-1 2,000/2,025/500-dia fp-none → **3× pass on d3, ৳1,380**, checkpoint 9/30, 60 on 10/8, gaps 9/21-24 + 9/28 only, no stockpile draws; startDay-5 2,000-dia → 3× pass on d5, ৳1,380 (was 4 passes / ৳1,660); fp-all-claimed keeps the d1 pass only where dropping it genuinely costs more (top-up ৳454 > pass ৳200). The **0-dia day-1 plan improved** (৳3,700 → ৳3,415: checkpoint d16 → d15, d16 emergency pass and 9/30 idle gone, final push 4 → 2 singles) — pin re-pinned `654e01876a1d4f0f` → `9e06d324be8a13ec` with rationale. `verify:aspirants`: **PASS 144 aspirants** (60 draws, 0 warnings, funded, tiers monotonic) + 6 byte-identical non-aspirants; `next lint` (3 pre-existing `<img>` warnings) + `next build` pass.
- Docs: `docs/architecture.md` (simulator paragraph), `docs/features.md` (draw-pacing + pricing), `docs/aspirants2026_status.md` (§3 resolved block), pin comment in `scripts/verify-aspirants.mjs`.


### Aspirants: lazy-cadence pacing applied (Sep 21 2026) — RESOLVED
- **Fix applied** in `simulateAspirantsFixedShape`: accumulation days now skip their discounted daily when (a) `anchorHold` — drawing would cross 40 before the first accumulation day (`firstAccDay = lastPhaseEnd + 1`), or (b) `spareHold` — remaining calendar capacity (1/day slots + future phase tokens + reserved 10x) already covers the target; both gated by `canResume` (tomorrow must still afford a daily) so balance-poor plans are byte-identical (0-dia hash pinned `7fe7408383add391`). The 10x window is now `day >= firstAccDay` (old mid-phase early-fire path removed).
- **Sub-row refinement**: on the 10x day, free token claims + 10x + packs move into `row.subrow` so `row.main` reads exactly 40. `attachAspirantsSubrowSplit` publishes `main + subrow === row` on every summed field; `ScheduleTable.jsx` / `exporter.js` / `reportBug.js` read the split (note-matching survives as the stale-plan fallback only).
- **Report (verified)**: 144 aspirants plans (60 draws, 0 warnings, funded, tiers monotonic) + 6 byte-identical non-aspirants; the `verify-aspirants.mjs` grid now also asserts rich-start pacing (dia ≥ 2000, startDay 1): checkpoint lands on day 15 = Sep 30, 10x same-day → 50, last draw on day 23 = Oct 8 (no idle tail). BDT shifts by the honest price of pacing (~+150–300 on rich cells, e.g. dia 2,025 → ৳1,380). `next lint` (3 pre-existing `<img>` warnings) + `next build` pass.
- Docs: `docs/architecture.md` (lazy-cadence simulator paragraph), `docs/features.md` (draw-pacing line), `docs/aspirants2026_status.md` (reference shape confirmed).



### Aspirants: same-day 10x sub row + main-row-only accounting (Sep 20 2026)
- **Problem (user report)**: the 40-checkpoint landed on 10/1 but the 50th was drawn the NEXT day (10/2) — the 10x ate 10/2's daily-draw slot, so the 51st draw only started on 10/3. The checkpoint row's numbers also merged both units: the 40th-draw row read `diaAdd=1270 / spent=1155 / left=137` instead of its own `100 / 105 / 17`.
- **Fix (1) — same-day 10x** (`lib/planOrchestrator.js`): the funding loop's `tenxDay` is now the organic 40-checkpoint day **itself** (was `checkpoint + 1`) and the cadence keeper's scope stops **before** that day (`r.day < tenxDay`), so the checkpoint day is funded by the 10x block alone (no double top-up). The simulator's in-loop gate fires the 10x right after the daily draw that reaches 40 → day 16 (10/1) = 11 draws (1 daily + 10 10x, cum 40 then 50) and day 17 (10/2) keeps its daily.
- **Fix (2) — main/sub-row split** (`attachAspirantsSubrowSplit`): the 10x day publishes `row.main` (the 40th-draw row: its own draws/diaSpent/diaAdd/diaRecharged/notes and a balance built only from its own adds/subtracts) + `row.subrow` (`If no bingo at 40`: the 10x and the packs bought that day), with `main + subrow === row` on every summed field. Packs bought that day ARE the 10x funding (sub row); every other recharge of the day is the main row's; if the day's own recharge can't cover its own draw, just enough pack money shifts to the main row to keep it ≥ 0. `simulateAspirantsFixedShape` / `buildAspirantsPlanWithPacks` return `firstTenxDay` (the row the 10x actually landed on) and `checkpointDay`/`tenxDay`/`tailDay` are re-read off the final sim rows, so the anchors can never drift from the schedule.
- **Consumers**: `ScheduleTable.jsx`, `exporter.js` and `reportBug.js` read the published split (the old note-matching survives only as the fallback for stale cached plans).
- **Report (day-1, 0 dia, fp none)**: Oct 1 main row = `1 draw / 105 spent / 100 added (1× weekly pass: 80 instant + 20 drip) / 17 left`, sub row = `10 draws / 1,050 spent / 1,170 added (fp_50 + fp_500)` — the hand plan's Oct 1 exactly. `verify:aspirants`: **PASS 144 plans** (60 draws, 0 warnings, funded, tiers monotonic) + 6 byte-identical non-aspirants plans; the new split invariants were mutation-tested (forcing the main row to absorb the 10x spend fails all 144). `next lint` (3 pre-existing `<img>` warnings) + `next build` pass.
- Docs: `docs/architecture.md` (staging paragraph corrected + new split section + plan-shape note), `docs/features.md` (draw-pacing line + schedule-table bullet).
...
### Aspirants tail under-recharge fix (Sep 19 2026)
- **Problem**: the aspirants tail branch sized injections as `max(0, shortfall − remainingDays × 105)` — crediting a 50%-off daily draw on every remaining day. When the residual fell below that projected saving (e.g. 175 < 735 on day-1 0-dia), the branch injected **nothing**, the `guard < 8` budget exhausted, and the plan shipped 175–734 diamonds short with a "N extra diamonds needed" warning while simultaneously over-buying (e.g. an `r_2195` left 4,816 dia idle).
- **Fix (1) — tail fallback** (`lib/planOrchestrator.js`): `needTail = max(0, shortfall − savings) || shortfall` — the daily-draw saving still sizes the injection where it applies, but the raw residual is always funded.
- **Fix (2) — no-progress escape** (`lib/planOrchestrator.js`): the loop now tracks whether the iteration injected anything; if no stage had something to fund it funds the recomputed residual directly on the tail day, so the guard cap can never exit with an unfunded plan. (Raising `guard` to 30 without this changed nothing — proof the dead end, not the iteration count, was the cause.)
- **Fix (3) — unified firstAccDay** (`lib/planOrchestrator.js`): one helper (`ASPIRANTS_ACC_OFFSET = 1`) shared by the loop and the staging fallback (they had drifted to `lastPhaseEnd + 2` vs `+ 1`). Offset 1 = day 15 for a day-1 start.
- **Report (verified)**: day-1/0-dia ৳3,615 → **৳3,415**, day-4 ৳9,601 → **৳9,390**, day-10 ৳10,623 → **৳10,542**; all 108 grid cells (dia 0–4,000 × start day 1/4/10/17/21/23 × FP unclaimed/claimed) pass with 0 warnings, 60 draws, `diaLeft ≥ 0`. The 6 non-aspirants regression hashes are byte-identical. Deliberate consequence: reported BDT **rises** wherever the old plan was unfundable — the old figures priced diamonds the schedule never bought.
- **Verify**: `npm run verify:aspirants` (new `scripts/verify-aspirants.mjs` + `scripts/next-alias-hook.mjs`, added to `package.json`). `npm run lint` → 3 pre-existing `<img>` warnings only. `npm run build` → EXIT=0.
- Docs: `docs/aspirants2026_status.md`, `docs/aspirants_ideal_plan.md`, `docs/architecture.md` (new "Aspirants (bingo) end-game funding" section).
