...
## Recent Changes
### Session checkpoint (Sep 21 2026) — pacing issue found, fix planned next
- **State**: the same-day-10x + main/sub-row split work (entry below) is verified (144 aspirants + 6 byte-identical non-aspirants, lint, build) and committed here as a checkpoint. Also riding along: a `PackRecommendation.jsx` display fix — packs that report an aggregated `diaTotal` now show `pack.dia` instead of `count × dia` (a 500-dia entry ×3 was reading 1,500 dia in the recommended-recharge list).
- **New problem (user report, probes confirmed)**: pacing is only correct for the 0-dia day-1 plan. Rich starts over-draw: `simulateAspirantsFixedShape`'s accumulation mode takes the discounted 1x on EVERY affordable gap/post-spend day, so at 2,000 dia the total hits 39 by day 11, phase-2's spend-task daily makes the 40th draw land inside phase 2 (d12/9/27) at cum 42, the same-day 10x → 52, the tail compresses to 10/5 and 10/6–10/8 sit empty. The 0-dia plan only paces "right" because those days starve — an accident of funding, not a rule. Probe baseline (day-1, no fp): 0-dia = 19 by d9, 40 exactly on d16, 60 on d23 (push 3); 2,000-dia = 23 by d9, 42→52 on d12, 60 on d20, d21–23 empty.
- **Two report corrections**: at 2,000 dia the day-1 weekly pass is ALREADY dropped in this code (3× pass on d3, none on d1, ৳1,236, all confidences) — the screenshot's day-1 pass was the deployed pre-fix build. And the startDay-5 over-recharge (4× passes carry 400 recharge vs the 250 threshold) is passes-as-funding, user accepted as minor.
- **Planned fix (next commit)**: lazy-cadence hold in the sim — an accumulation day skips its discounted daily when (a) drawing would cross the 40 checkpoint before the first accumulation day (lastPhaseEnd+1), or (b) future capacity (remaining 1/day slots + future phase tokens + reserved 10x) already covers the target, gated by a smooth-balance funding guard (skip only if tomorrow can still draw) so balance-poor plans stay byte-identical. Plus split refinement: on the 10x day, token claims move into the sub row so the main checkpoint row reads exactly 40. Target shape for rich day-1 plans: 40 exactly on 9/30, sub row 50, 51 on 10/1, 60 exactly on 10/8, no empty days.
